...
let userModel = (() => {
    ...
    function getAllUsers() {
        sql = ''
            + 'use users;\n'
            + 'select user_id as userId, name, email, user_created_at as createdAt, privilage, is_active as isActive from users;';
    ...
    function deleteUserFromDb(userId) {
        ...
        return new Promise((resolve, reject) => {
            dbPool.queryProm(sql)
            .then(results => {
                let isUserDeleted = results.affectedRows > 0;
                resolve(isUserDeleted);
            })
            .catch(err => {
                reject(err);
            })
    ...
    function activateUserChangeTmpPassword(userId, newPassword) {
        userId = dbPool.escape(userId);
        ...
        sql = ''
            + 'use users;\n'
            + `update users\n` 
                + `set password_hash = ${newPasswordHash}, is_active = 1\n`
            + `where user_id = ${userId};\n`
