...
adminAuthRouter.get(config.restEndpoints.admin.auth.logInView, redirectIfLoggedIn, moveSessionBodyToReq, getLoginView);
adminAuthRouter.get(config.restEndpoints.admin.auth.register.registerView, requireBeAuthenticated, moveSessionBodyToReq, getRegisterView);
adminAuthRouter.post(config.restEndpoints.admin.auth.register.createUser, requireBeAuthenticated, upload.single('userAvatarImg'), registerUser);
adminAuthRouter.post(config.restEndpoints.admin.auth.changeTmpPwd, requireBeTmpAuthenticated, changeTmpPwd);
adminAuthRouter.get(config.restEndpoints.admin.auth.changeTmpPwdView, requireBeTmpAuthenticated, getChangeTmpView);

adminAuthRouter.get(config.restEndpoints.admin.auth.logOut, requireBeAuthenticated, (req, res, next) => {
    req.logout();
...
adminAuthRouter.post(config.restEndpoints.admin.auth.oAuth.local.login, redirectIfLoggedIn, passport.authenticate('local' , { 
...
adminAuthRouter.get(config.restEndpoints.admin.auth.oAuth.failureRed, redirectIfLoggedIn, (req, res) => {
...
adminAuthRouter.get(config.restEndpoints.admin.auth.oAuth.succRed, requireBeAuthenticated, (req, res) => {
...
adminAuthRouter.post(config.restEndpoints.admin.auth.tmpOAuth.local.login, redirectIfLoggedIn, passport.authenticate('local' , { 
...
adminAuthRouter.get(config.restEndpoints.admin.auth.tmpOAuth.failureRed, redirectIfLoggedIn, (req, res) => {
...
adminAuthRouter.get(config.restEndpoints.admin.auth.tmpOAuth.succRed, requireBeTmpAuthenticated, (req, res) => {
